...
static uint16_t analogRead(uint8_t channel);
...
static uint16_t analogRead(uint8_t channel)
{
    ...
    {
        ADC_INIT(64, INTERNAL); // we set [ADC_SINGLE_INIT= 0] inside the ADC_INIT() function to ensure only one execution of ADC_INIT()
